...
function getMembershipInfo($userID){
  ...
  $stmt = $conn->prepare("select ui.user_id as user_id, ui.user_name as user_name, ui.point as point, count(*) as bookCnt, sum(datediff(bl.checkout_date, bl.checkin_date)) as sleepDays, sum(pl.amount) as amount, sum(pl.vat) as vat from user_information ui, booking_log bl, pay_log pl where ui.user_id=? and ui.user_id=bl.user_id and
  bl.pay_log_id=pl.pay_log_id");
