...
function getRoomList(){
  ...
  while($row = mysqli_fetch_assoc($res)){
    ...
    array_push($jsonObj['body'], array('room_type_id' => $row['room_type_id'], 'room_name' => $row['room_name'], 'base_price' => $row['base_price'], 'addfee_price' => $row['addfee_price'], 'description' => $row['description'], "isbook" => $isBook, "count" => $row2['count(*)']));
    $count++;
  }

  if($count == 0){
    $jsonObj['header'] = ['result' => 'fail', 'cause' => 'not_found', 'count' => $count];
  }else{
